...
public class CreateTableCompanyDAO extends _DAOSuper {
	public CreateTableCompanyDAO(String projectName) {
		init();
		Scanner in = new Scanner(System.in);
		System.out.println("회사 이름을 입력하시오.");
		create(projectName, in.nextLine());
	}
	
	// 입력값은 숫자로 하고 출력 시 문자로 받는것 고민 필요....
	public void create(String projectName, String companyName) {
		if (con()) {
			...
			try {
				String sqlCompany = "create table company ("
						...
						+ "constraint com_project_name_"+projectName+" foreign key (com_project_name) references "+projectName+" (project_name)"
						+ ")";
